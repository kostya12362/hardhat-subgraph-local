...
export function createPoolFunctions({
  ...
  async function swapToSqrtPrice(
    ...
    await inputToken.approve(swapTarget.address, ethers.MaxUint256)
  ...
  const mint: MintFunction = async (recipient, tickLower, tickUpper, liquidity) => {
    await token0.approve(swapTarget.address, ethers.MaxUint256)
    await token1.approve(swapTarget.address, ethers.MaxUint256)
