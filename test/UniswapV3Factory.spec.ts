...
describe('Dex223Factory', () => {
  ...
  it('factory bytecode size', async () => {
    expect(((await waffle.provider.getCode(factory.target)).length - 2) / 2).to.matchSnapshot()
  })
  ...
  it('pool bytecode size', async () => {
    ...
    const poolAddress = getCreate2Address(String(factory.target), TEST_ADDRESSES, FeeAmount.MEDIUM, poolBytecode)
  ...
  async function createAndCheckPool(
    ...
    const create2Address = getCreate2Address(String(factory.target), tokens, feeAmount, poolBytecode)
    // TODO pool require 4 addresses
  ...
  describe('#createPool', () => {
    ...
    it('fails if token a is 0 or token b is 0', async () => {
      ...
      await expect(factory.createPool(ethers.ZeroAddress, ethers.ZeroAddress, FeeAmount.LOW)).to.be.revertedWith(
        ''
      )
    })
